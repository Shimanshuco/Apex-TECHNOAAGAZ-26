...
const Sidebar: React.FC<SidebarProps> = ({ navItems, socialLinks = [], activeNav, onNavChange, mobileOpen, onMobileToggle, className = '' }) => {
  const { user } = useAuth();
  ...
  return (
    <>
      ...
      <nav className={`
        fixed left-0 top-15 h-full z-40
        bg-transparent
        hidden md:flex
        w-28 py-6 pt-32 px-2
      ...
      >
        ...
        {user ? (
          <button
            ...
            onClick={() => { navigate('/profile'); onMobileToggle(false); }}
          >
            My Profile
          </button>
        ) : (
          <button
            ...
            onClick={() => { navigate('/register'); onMobileToggle(false); }}
          >
            Register Now
          </button>
        )}
